...
namespace MineImatorSimplyRemadeNuxi.core.project;
...
public static class ProjectManager
{
	...
	public static string GetLastNewProjectFolder()
	{
		try
		{
			var path = Path.Combine(App.GetUserDataPath(), UserPrefsFileName);
	...
	private static List<RecentProjectEntry> LoadRecentProjects()
	{
		try
		{
			var path = Path.Combine(App.GetUserDataPath(), RecentProjectsFileName);
	...
	public static bool NewProject(string projectFolder)
	{
		...
		Console.WriteLine($"New project created at '{projectFolder}'");
	...
	public static bool OpenProject(string projectFilePath)
	{
		...
		try
		{
			...
			if (data == null)
			{
				Console.WriteLine("Failed to deserialize project file");
				return false;
			}
	...
	public static string ImportAsset(string sourcePath)
	{
		...
		try
		{
			...
			Console.WriteLine($"Imported asset '{fileName}' → '{destPath}'");
			return destPath;
		}
		catch (Exception ex)
		{
			Console.WriteLine($"Failed to import asset: {ex.Message}");
			return "";
		}
	}
	...
	public static void RemoveAsset(string relativePath, bool deleteFile = false)
	{
		...
		if (deleteFile)
		{
			...
			if (File.Exists(fullPath))
			{
				...
				catch (Exception ex) { Console.WriteLine($"Failed to delete asset: {ex.Message}"); }
			}
		}
	...
	private static void CollectSceneState()
	{
		...
		_currentData.CurrentFrame = Timeline.Instance?.CurrentFrame ?? 0;
		...
		var viewport = Program.App.Viewport;
		if (viewport == null) return;

		foreach (var child in viewport.GetChildren())
		{
			CollectSceneObjectRecursive(child, null);
		}
	}
	...
	private static void CollectProjectSettingsState()
	{
		...
		_currentData.Settings.FloorVisible = propPanel.Floor?.Visible ?? true;
	...
	/// </summary>
	private static void CollectProjectSettings(PropertiesPanel propPanel)
	{
		try
		{
	...
	/// </summary>
	private static void CollectBackgroundSettings(PropertiesPanel propPanel)
	{
		try
		{
			// Get background color from the property
			var bgColor = propPanel.BackgroundColor;
			var color = new Color(bgColor[0], bgColor[1], bgColor[2], bgColor[3]);
			_currentData.Settings.BackgroundColor = color.ToHex();
	...
	private static void CollectSceneObjectRecursive(SceneObject obj, string parentId)
	{
		...
		if (meshInstances.Count > 0 && meshInstances[0] != null && meshInstances[0].GetSurfaceCount() > 0)
		{
			...
			{
				entry.ExtraData["MatAlbedoColor"] = stdMat.AlbedoColor.ToHex();
				...
				entry.ExtraData["MatEmissionColor"] = stdMat.Emission.ToHex();
		...
		{
			entry.ExtraData["LightColor"] = lightObj.LightColor.ToHex();
	...
	private static void RestoreMaterialProperties(SceneObject obj, SceneObjectEntry entry)
	{
		...
		foreach (var meshInstance in meshInstances)
		{
			...
			{
				var material = meshInstance.SurfaceGetMaterial(i);
				StandardMaterial stdMat;

				if (material is StandardMaterial existingMat)
				{
					stdMat = existingMat;
				}
				else
				{
					...
					stdMat = new StandardMaterial();
					//meshInstance.SurfaceSetMaterial(i, stdMat);
				}
	...
	private static void ApplyCameraMaterial(CameraSceneObject cameraObj)
	{
		...
		//var material = GD.Load<StandardMaterial>("res://assets/mesh/CameraMaterial.tres");
		//if (material == null)
		{
		...
		foreach (var meshInstance in meshInstances.Where(meshInstance => meshInstance != null))
		{
			for (int i = 0; i < meshInstance.GetSurfaceCount(); i++)
			{
				//meshInstance.SurfaceSetMaterial(i, material);
			}
		}
	}
	...
	private static void CopyMiModelAssets(string mimodelSourcePath, string destFolder)
	{
		try
		{
			...
			foreach (var texRelPath in texturePaths)
			{
				...
				{
					Console.WriteLine($"Mimodel texture not found, skipping: '{texSourcePath}'");
					continue;
				}
	...
	private static void CopyMiObjectAssets(string miobjectSourcePath, string destFolder)
	{
		try
		{
			...
			{
				Console.WriteLine($"No resources section found in miobject '{miobjectSourcePath}'");
				return;
			}
			...
			foreach (var mimodelFilename in mimodelFilenames)
			{
				...
				{
					Console.WriteLine($"Referenced mimodel not found: '{mimodelSourcePath}'");
					continue;
				}
